...
| Excel按列头写（指定样式）             | Map<String, String>列表 | `new Xlsx().style("style.xlsx", CLASSPATH).fromBeans(titles, maps, new XlsxOptionFrom().titleStyle("A1").dataStyle("A2")).write(excel)` |
